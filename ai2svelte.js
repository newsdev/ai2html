// ai2svelte is a script for Adobe Illustrator that converts your Illustrator document into html and css.
...
// =====================================
// How to install ai2svelte
// =====================================

// - Move the ai2svelte.js file into the Illustrator folder where scripts are located.
// - For example, on Mac OS X running Adobe Illustrator CC 2014, the path would be: // Adobe Illustrator CC 2014/Presets/en_US/Scripts/ai2svelte.jsx

// =====================================
// How to use ai2svelte
// =====================================
...
// - Run the script by choosing: File > Scripts > ai2svelte
// - Go to the folder containing your Illustrator file. Inside will be a folder called ai2svelte-output.
...
// ================================================
// ai2svelte and config settings
// ================================================

// These are base settings that are overridden by text block settings in
// the .ai document and settings contained in ai2svelte-config.json files
  var defaultSettings = {
    "namespace": "",
    ...
    "html_output_path": "ai2svelte-output/",
    "html_output_extension": ".svelte",
    "image_output_path": "ai2svelte-output/",
    "image_source_path": "",
    ...
    "use_container_media_query": true, // Use media query in addition to svelte js
    ...
    // List of settings to include in the "ai2svelte-settings" text block
    "settings_block": [
      "settings_version",
      "image_format",
      "responsiveness",
...
// These settings override the default settings in NYT preview/birdkit projects
  var nytOverrideSettings = {
    ...
    "min_width": 280, // added as workaround for a scoop bug affecting ai2svelte-type graphics
    "accessibility": true,
    "settings_block": [
      "settings_version",
      "responsiveness",
  ...
  function render(settings, customBlocks) {
    ...
    forEachUsableArtboard(function(activeArtboard, abIndex) {
      ...
      if (settings.output == 'one-file' &&
        settings.project_type == 'ai2svelte' &&
...
// Add internal functions to module.exports for testing in Node.js
  ...
  function isTestedIllustratorVersion(version) {
    ...
    return majorNum >= 22 && majorNum <= 28; // Illustrator CC through 2024
  }
...
//   formatted text blocks
  function initSpecialTextBlocks() {
    var rxp = /^ai2(svelte|html)-(css|js|html|settings|text|html-before|html-after)\s*$/;
    var settings = null;
    var code = {};
    forEach(doc.textFrames, function(thisFrame) {
      ...
      if (type != 'settings' && (thisFrame.name == 'ai2svelte-settings' || thisFrame.name == 'ai2html-settings')) {
        thisFrame.name = '';
      }
...
// Trigger errors and warnings for some common problems
  ...
  function detectTimesEnv(blockSettings) {
    ...
    if (nytFonts && !nytProjectEnv) {
      if (confirm("You seem to be running ai2svelte outside of a NYT graphics project.\nContinue in non-NYT mode?", true)) {
        nytEnv = false;
      } else {
    ...
    if (!nytFonts && nytProjectEnv) {
      if (confirm("Your system is missing the NYT fonts.\nContinue?", true)) {
        ...
        error("Install the NYT Fonts and then re-run ai2svelte.");
      }
    }
    ...
    if (nytEnv && blockSettings && detectUnTimesianSettings(blockSettings)) {
      error('The settings block is incompatible with NYT Preview. Delete it and re-run ai2svelte.');
    }
  ...
  function applyBirdkitSettings(settings, projectType) {
    ...
    var isEmbed = projectType == 'ai2svelte' || // manual setting from text block
      pkg.projectTemplate == '@newsdev/template-ai2svelte' ||
      // (deprecated) read from local ai2svelte-config.json file
      readConfigFileSettings().project_type == 'ai2svelte' ||
...
// assumes three-part version, e.g. 1.5.0
  ...
  function applyTimesSettings(settings, blockSettings) {
    ...
    if (detectBirdkitEnv()) {
      ...
      if (!yamlConfig) {
        warn('ai2svelte is unable to read the contents of config.yml');
      }
    ...
    if (!settings.project_type) {
      error("ai2svelte is unable to determine the type of this project.");
    } else if (settings.project_type != 'ai2svelte' && !folderExists(docPath + '../src/')) {
...
// Looks for settings file in the ai2svelte script directory and/or the .ai document directory
  function readConfigFileSettings() {
    var settingsFile = 'ai2svelte-config.json';
...
// with settings to override the default ai2svelte settings.
...
// (e.g. undo Illustrator's automatic quote conversion, where applicable)
  ...
  /**
   * Attempts to get a layer by one of two names. Useful for ai2svelte and ai2html backwards compatibility.
   * @param {string} name1 - The first layer name to search for.
   * @param {string} name2 - The second layer name to search for.
   * @returns {Layer} The found Illustrator Layer.
   * @throws {Error} If neither layer is found.
   */
  function getLayerByNames(name1, name2) {
    var layer;
    try {
      // Try to get the layer by the first name
      layer = doc.layers.getByName(name1);
    } catch (error) {
      try {
        // If the first name fails, try the second name
        layer = doc.layers.getByName(name2);
      } catch (error) {
        // If both names fail, throw a new error
        throw new Error("Neither '" + name1 + "' nor '" + name2 + "' layers were found.");
      }
    }
    return layer;
  }
  ...
  /**
   * Attempts to get a textFrame by one of two names. Useful for ai2svelte and ai2html backwards compatibility.
   * @param {string} name1 - The first textFrame name to search for.
   * @param {string} name2 - The second textFrame name to search for.
   * @returns {TextFrame} The found Illustrator TextFrame.
   * @throws {Error} If neither textFrame is found.
   */
  function getTextFrameByNames(name1, name2) {
    var block;
    try {
      // Try to get the textFrame by the first name
      block = doc.textFrames.getByName(name1);
    } catch (error) {
      try {
        // If the first name fails, try the second name
        block = doc.textFrames.getByName(name2);
      } catch (error) {
        // If both names fail, throw a new error
        throw new Error("Neither '" + name1 + "' nor '" + name2 + "' textFrames were found.");
      }
    }
    return block;
  }
  ...
  function createSettingsBlock(settings) {
    ...
    var top         = bounds[1];
    var settingsLines = ["ai2svelte-settings"];
    ...
    try {
      layer = getLayerByNames("ai2svelte-settings", "ai2html-settings");
      layer.locked = false;
    } catch(e) {
      layer = doc.layers.add();
      layer.zOrder(ZOrderMethod.BRINGTOFRONT);
      layer.name  = "ai2svelte-settings";
    }
    ...
    textArea.name = 'ai2svelte-settings';
...
// Update an entry in the settings text block (or add a new entry if not found)
  function updateSettingsEntry(key, value) {
    var block = getTextFrameByNames("ai2svelte-settings", "ai2html-settings");
    var entry = key + ': ' + value;
    var updated = false;
    var lines;
    if (!block) return;
...
//
  function exportImage(imgName, format, ab, masks, layer, settings) {
    ...
    var imgDir = pathJoin(getImageFolder(settings), getDocumentName());
    checkForOutputFolder(imgDir, 'image_output_path');
    
    var outputPath = pathJoin(getImageFolder(settings), getDocumentName(), imgFile);
...
// Create an <img> tag for the artboard image
  function generateImageHtml(imgFile, imgId, imgClass, imgStyle, ab, settings) {
    var imgDir = settings.image_source_path ? pathJoin(settings.image_source_path, getDocumentName()) : '',
...
// Add ab content to an output
  ...
  function generateArtboardCss(ab, cssRules, settings) {
    ...
    if (isTrue(settings.use_container_media_query)) {
      var visibleRange = getArtboardVisibilityRange(ab, settings);
...
// Get CSS styles that are common to all generated content
  function generatePageCss(containerId, settings) {
    ...
    if (settings.clickable_link !== '') {
      css += blockStart + ' .' + nameSpace + 'ai2svelteLink {';
  ...
  function getCommonOutputSettings(settings) {
    ...
    return {
      ai2svelte_version: scriptVersion,
      project_type: 'ai2svelte',
      min_width: range[0],
      max_width: range[1],
      tags: 'ai2svelte',
...
// Create a settings file (optimized for the NYT Scoop CMS)
  function generateYamlFileContent(settings) {
    var o = getCommonOutputSettings(settings);
    var lines = [];
    lines.push('ai2svelte_version: ' + scriptVersion);
    if (settings.project_type) {
...
// Write an HTML page to a file for NYT Preview
  function outputLocalPreviewPage(textForFile, localPreviewDestination, settings) {
    ...
    settings.ai2sveltePartial = textForFile; // TODO: don't modify global settings this way
  ...
  function addCustomContent(content, customBlocks) {
    ...
    // TODO: assumed JS contained in <script> tag -- run this onMount
    if (customBlocks.js) {
...
// Wrap content HTML in a <div>, add styles and resizer script, write to a file
  function generateOutputHtml(content, pageName, settings) {
    ...
    htmlFileDestinationFolder = docPath + settings.html_output_path + getDocumentName() + "/";
    checkForOutputFolder(htmlFileDestinationFolder, 'html_output_path');
    htmlFileDestination = htmlFileDestinationFolder + (settings.html_file_name || pageName) + settings.html_output_extension;
    
    // 'index' is assigned upstream now (where applicable)
    // if (settings.output == 'one-file' && settings.project_type == 'ai2svelte') {
